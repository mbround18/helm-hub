...
if [ ! -x /app/helm-hub ]; then
    echo "[entrypoint] ERROR: /app/helm-hub is missing or not executable."
    exit 1
fi
...
if [ "${CLAMAV_ENABLED}" = "true" ]; then
    ...
    echo "[entrypoint] CLAMAV_ENABLED=false — skipping freshclam/clamd startup."
fi

# ── 4. Launch the Rust service ────────────────────────────────────────────────
echo "[entrypoint] Starting Helm Hub backend: ${APP_CMD}"
# exec replaces this shell process; tini continues to own PID 1.
exec su -s /bin/sh helmhub -c "exec ${APP_CMD}"
