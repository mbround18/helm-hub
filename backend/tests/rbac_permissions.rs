...
async fn create_test_user(server: &TestServer, username: &str, password: &str) -> (String, String, String, Uuid) {
...
async fn test_owner_can_promote_to_any_role() {
    ...
    let (_user1, _pass1, token1, _id1) = create_test_user(&server, &format!("user-{}", Uuid::new_v4()), "pass123").await;
    let (_user2, _pass2, _token2, id2) = create_test_user(&server, &format!("user-{}", Uuid::new_v4()), "pass123").await;
...
async fn test_admin_can_only_promote_to_user_and_admin() {
    ...
    let (_user, _pass, token, _user_id) = create_test_user(&server, &format!("user-{}", Uuid::new_v4()), "pass123").await;
    ...
    assert!(response.status_code() == StatusCode::FORBIDDEN || response.status_code() == StatusCode::UNAUTHORIZED);
}
...
async fn test_user_cannot_promote_anyone() {
    ...
    let (_user, _pass, token, _id) = create_test_user(&server, &format!("user-{}", Uuid::new_v4()), "pass123").await;
    let (_user2, _pass2, _token2, id2) = create_test_user(&server, &format!("user-{}", Uuid::new_v4()), "pass123").await;
...
async fn test_cannot_self_promote() {
    ...
    let (_user, _pass, token, user_id) = create_test_user(&server, &format!("user-{}", Uuid::new_v4()), "pass123").await;
...
async fn test_user_can_delete_own_chart() {
    ...
    let (username, _pass, token, _user_id) = create_test_user(&server, &format!("user-{}", Uuid::new_v4()), "pass123").await;
...
async fn test_user_cannot_delete_other_users_chart() {
    ...
    let (_user1, _pass1, token1, _id1) = create_test_user(&server, &format!("user-{}", Uuid::new_v4()), "pass123").await;
    let (user2, _pass2, _token2, _id2) = create_test_user(&server, &format!("user-{}", Uuid::new_v4()), "pass123").await;
...
async fn test_chart_deletion_requires_authentication() {
    ...
    let (username, _pass, _token, _user_id) = create_test_user(&server, &format!("user-{}", Uuid::new_v4()), "pass123").await;
