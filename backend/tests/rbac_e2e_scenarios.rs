...
async fn scenario_user_cannot_promote_anyone() {
    ...
    let user1_token = create_user_with_token(&server, &format!("user-{}", Uuid::new_v4()), "pass123").await;
    ...
    let user2_id = Uuid::parse_str(
        user2_response.json::<Value>()["user"]["id"].as_str().unwrap()
    ).unwrap();
...
async fn scenario_privilege_escalation_prevented() {
    ...
    let _user_token = create_user_with_token(&server, &format!("user-{}", Uuid::new_v4()), "pass123").await;
    ...
    let response = server
        .get("/api/admin/users")
        .add_header("Authorization", format!("Bearer {}", malicious_token).as_str())
        .await;
...
async fn scenario_multiple_users_isolation() {
    ...
    // Create 3 users
    let _user1_token = create_user_with_token(&server, &format!("user-{}", Uuid::new_v4()), "pass123").await;
    let user2_token = create_user_with_token(&server, &format!("user-{}", Uuid::new_v4()), "pass123").await;
    let _user3_token = create_user_with_token(&server, &format!("user-{}", Uuid::new_v4()), "pass123").await;
...
async fn scenario_analytics_access_control() {
    ...
    let user_token = create_user_with_token(&server, &format!("user-{}", Uuid::new_v4()), "pass123").await;
