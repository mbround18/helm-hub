...
fn test_no_colon_route_parameters() {
    ...
    println!("✅ All {} routes use correct Axum syntax with curly braces", routes.len());
}
...
fn test_all_routes_have_parameters_in_braces() {
            ...
            if chars[i] == '{' {
                ...
                if found_close {
                    ...
                    invalid_routes.push(format!(
                        "❌ Route '{}' has unclosed parameter brace",
                        route
                    ));
                    break;
                }
    ...
    println!("✅ All {} parameterized routes use correct curly brace syntax", parameterized_routes.len());
}
