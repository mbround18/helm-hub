...
use crate::{db::DbConn, error::AppError, services::auth_providers};
...
pub async fn otel_endpoint(conn: &mut DbConn) -> Option<String> {
    ...
    if let Some(endpoint) =
        auth_providers::opt_setting(conn, auth_providers::KEY_OTEL_ENDPOINT).await
        && !endpoint.is_empty()
    {
...
pub async fn grafana_config(
    ...
    let token_encrypted =
        auth_providers::opt_setting(conn, auth_providers::KEY_GRAFANA_API_TOKEN).await;
    let token = match token_encrypted {
        Some(enc) => crate::services::token_crypto::decrypt_token(&enc, encryption_key)?,
        None => return Ok(None),
    };
...
pub fn embed_panel_url(config: &GrafanaConfig, owner: &str, chart: &str) -> String {
    let dashboard_name = format!("helm-hub-{}-{}", owner, chart);
    format!(
