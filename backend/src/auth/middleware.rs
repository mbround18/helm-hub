...
pub async fn require_auth(
    ...
    let claims = if raw.starts_with("hhub_") {
        resolve_api_token(&state, raw).await?
    } else {
...
async fn resolve_api_token(state: &AppState, raw_token: &str) -> Result<Claims, AppError> {
    ...
    let expires_at: DateTime<Utc> = token.expires_at;
